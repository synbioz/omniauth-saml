...
* `:default_relay_state` - The value to use as default `RelayState` on single log outs. The
  value can be a URL, or can be configured as a string or a callback that optionally takes
  the request and results in a URL. This would be on the service provider. Optional.
